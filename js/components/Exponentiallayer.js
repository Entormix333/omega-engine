class ExponentialLayer
{
    constructor()
    {
        ...
        this.upgrades = {
            ...
            level => Decimal.pow(1e5, level.pow(1.5)).mul(1000),
            level => new Decimal (1).add(game.metaLayer.layer.mul(level).pow(level.div(10)).add(1).log(10).pow(0.8).div(10)),
            {getEffectDisplay: effectDisplayTemplates.numberStandard(10, "^")}
            ),
            ...
            level => Decimal.add(1, level).pow(level.add(1).log(10).add(1)).pow(100),
            {getEffectDisplay: effectDisplayTemplates.numberStandard(3, "^")}
            ),
            ...
            level => new Decimal (1000).pow(level.pow(6)).mul(1000).floor().div(10),
            {getEffectDisplay: effectDisplayTemplates.numberStandard(3, "÷")}
            ),
        }
    }
    ...
    {
        if(game.metaLayer.layer.lte(new Decimal("1.00e33"))) return new Decimal(0);
        return Decimal.log10(game.metaLayer.layer).sub("33")
    }
